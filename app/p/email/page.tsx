...
import { CheckCircle, Copy, Send, Star, Mail, ArrowRight, Clock, MousePointerClick, TrendingUp } from "lucide-react";

const BEFORE_AFTER = {
  bad: { subject: "Welcome to our newsletter!", body: "Hi there,\n\nThanks for signing up for our newsletter. We'll send you updates about our product and company news.\n\nStay tuned!\n\nThe Team" },
  good: { subject: "You're in. Here's what to expect (and a gift)", body: "Hi Sarah,\n\nQuick note to say welcome — and thanks for trusting us with your inbox.\n\nOver the next week, I'll send you 3 things:\n• The exact workflow our top customers use (Tuesday)\n• A case study: how one founder 3x'd her email revenue (Thursday)\n• Your first month checklist (Saturday)\n\nBut first — here's the SEO audit template I promised.\n[Download: seo-audit-template.pdf]\n\nTalk soon,\nJess @ SEO Spark" },
};
...
const TESTIMONIALS = [
  ...
  { name: "Nina Okafor", role: "Marketing Lead, B2B SaaS", text: "We A/B tested our old sequence against AI-generated one. The new sequence had 2.3x higher click rate. The difference? Every email actually delivers value before asking for anything.", stars: 4 },
];
...
export default function EmailPage() {
  ...
  async function handleSubmit(e: React.FormEvent) { e.preventDefault(); if (!product.trim()) return; setLoading(true); setError(""); setResult(null);
    try { const res = await fetch("/p/email/api/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ product, audience, tone, benefit }) }); const data = await res.json(); if (!res.ok) throw new Error(data.error); setResult(data); } catch (err: any) { setError(err.message); } finally { setLoading(false); } }
  ...
  return (
    <main className="bg-white">
      <section className="max-w-3xl mx-auto px-4 sm:px-6 pt-12 sm:pt-24 pb-10 text-center">
        ...
        <h1 className="text-3xl sm:text-5xl font-extrabold text-gray-900 mb-3 leading-tight">Welcome emails that <span className="text-amber-600">actually convert.</span></h1>
        <p className="text-gray-500 max-w-lg mx-auto mb-8 text-sm sm:text-base">5 emails. One welcome sequence. Generated in 30 seconds. Outsourcing this costs $200–500.</p>
      ...
      {!result && (
        <section className="max-w-4xl mx-auto px-4 sm:px-6 pb-16">
          <div className="text-center mb-8">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Your welcome email deserves better than this.</h2>
            <p className="text-gray-500 text-sm">Left: the email most businesses send. Right: what we generate.</p>
          </div>
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
              <p className="text-xs font-semibold text-red-600 mb-3">❌ Generic Welcome Email</p>
              <div className="bg-white rounded-xl p-4 text-sm">
                <p className="font-medium text-gray-400 mb-2 text-xs">Subject: {BEFORE_AFTER.bad.subject}</p>
                <p className="text-gray-400 whitespace-pre-wrap leading-relaxed text-xs">{BEFORE_AFTER.bad.body}</p>
              </div>
              <ul className="mt-3 space-y-1 text-xs text-red-600">
                <li>· No personalization</li><li>· Zero value delivered</li><li>· "Stay tuned" = instant delete</li>
              </ul>
            </div>
            <div className="bg-amber-50 border-2 border-amber-200 rounded-2xl p-5">
              <p className="text-xs font-semibold text-amber-700 mb-3">✅ AI-Generated Welcome Sequence</p>
              <div className="bg-white rounded-xl p-4 text-sm">
                <p className="font-medium text-gray-900 mb-2 text-xs">Subject: {BEFORE_AFTER.good.subject}</p>
                <p className="text-gray-700 whitespace-pre-wrap leading-relaxed text-xs">{BEFORE_AFTER.good.body}</p>
              </div>
              <ul className="mt-3 space-y-1 text-xs text-amber-700">
                <li>· Uses their name</li><li>· Sets clear expectations</li><li>· Delivers value immediately (free template)</li>
              </ul>
            </div>
          </div>
        </section>
      ...
      <section className="border-t border-gray-100 py-12">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-3 gap-4 text-center mb-10">
            {[{ v: "5", l: "Emails/sequence" },{ v: "30s", l: "Generation time" },{ v: "34%", l: "Avg open rate" }].map(s => (
              <div key={s.l} className="bg-amber-50 rounded-xl p-4"><div className="text-2xl sm:text-3xl font-bold text-amber-600">{s.v}</div><p className="text-xs sm:text-sm text-gray-500 mt-1">{s.l}</p></div>
            ))}
          </div>
      ...
      <section className="bg-gray-50 py-8">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-1.5">
            {[["/","📝 Blog Posts"],["/p/proposal","🎯 Proposals"],["/p/faq","❓ FAQ"],["/p/product","🛍 Products"],["/p/cold-email","📧 Cold Email"]].map(([h,l]) => (
              <a key={h} href={h} className="bg-white border border-gray-200 rounded-lg px-2.5 py-1.5 text-xs text-gray-500 hover:border-amber-300 hover:text-amber-600 transition-all">{l}</a>
            ))}
          </div>
        </div>
      </section>
      <footer className="text-center py-6 text-xs text-gray-400">Email Sequence Generator — from SEO Spark. Welcome emails that convert.</footer>
    </main>
  );
}
